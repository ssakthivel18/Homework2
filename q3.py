...
Original file is located at
    https://colab.research.google.com/drive/11WeLxm7p7jTCzGAyt0eSK95VU50Fb0Fp
"""
...
class GaussianDiscriminator:
    ...
    def fit(self, X, y, num_classes):
      for k in range(num_classes):
        class_sample = X[np.where(y == k)]
        mean = np.mean(class_sample, axis=0)
        cov = np.cov(class_sample, rowvar=False)
    ...
    def predict(self, x):
        ...
        for i in range(x.shape[0]):
            probabilities = []
            for k, mean, cov in self.gaussians:
                probabilities.append(pdf(x[i], mean, cov))
